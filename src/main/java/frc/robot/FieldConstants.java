...
public class FieldConstants {
  ...
  private static final Distance trenchDelta =
  ...
  public static boolean underTrench(Pose2d pose) {
    boolean allianceTrench =
        pose.getMeasureX().lt(trenchFarLine) && pose.getMeasureX().gt(trenchCloseLine);
    boolean opponentTrench =
        pose.getMeasureX().lt(trenchFarLine.plus(neutralZoneLength))
            && pose.getMeasureX().gt(trenchCloseLine.plus(neutralZoneLength));
